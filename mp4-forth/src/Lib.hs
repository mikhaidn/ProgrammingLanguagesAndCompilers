...
initialDictionary =
  initArith ++ initComp ++ initIStackOp ++ initPrintOp
    ++ initCompileOp
...
cstackElse cstack@(("if", kif) : _) = Just $ ("else", id) : cstack
cstackElse _ = Nothing

-- I don't understand *why* this works, I tried to avoid using trans* vs in-line implementation
transConditional :: Transition -> Transition -> (ForthState -> ForthState)
transConditional kif kelse (i : is, d, o) =
  if i /= 0
    then kif (is, d, o)
    else kelse (is, d, o)
transConditional _ _ _ = underflow

cstackThen :: CStack -> Maybe CStack
cstackThen (("else", kelse) : ("if", kif) : (c, kold) : cstack) =
  Just $ (c, knew) : cstack
  where
    knew = (transConditional kif kelse) . kold
cstackThen (("if", kif) : (c, kold) : cstack) =
  Just $ (c, knew) : cstack
  where
    knew = (transConditional kif id) . kold
...
transUntil kloop state = aux (kloop state)
  where
    aux st@(i : is, d, o) =
      if i /= 0
      then (is, d, o) 
      else aux (kloop (is, d, o)) 
    aux _ = underflow  
